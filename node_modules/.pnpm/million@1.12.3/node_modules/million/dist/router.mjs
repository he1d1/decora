import { v as batch, q as startTransition, p as patch, c as createElement } from './chunks/render.mjs';
...
const normalizeRelativeURLs = (el, base) => {
  const hrefs = el.querySelectorAll(
    '[href^="./"], [href^="../"]'
  );
...
const request = async (url, options) => {
  return fetch(String(url), options).then(async (res) => {
  ...
  const submitHandler = (event) => {
    ...
    queueNavigation(() => {
      void navigate(url, selector, {
        method: el.method,
        redirect: "follow",
        body: !el.method || el.method.toLowerCase() === "get" ? `?${new URLSearchParams(body).toString()}` : JSON.stringify(body)
      });
    });
  };
